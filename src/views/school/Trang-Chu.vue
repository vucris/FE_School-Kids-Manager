...
async function onClickDownloadTemplate() {
    try {
        ...
        swalToast.fire({ icon: 'success', title: 'Đã tải file Excel mẫu' });
    } catch (e) {
        swalToast.fire({
            icon: 'error',
...
async function onConfirmImport() {
    ...
    try {
        ...
        swalToast.fire({
            icon: 'error',
            title: e?.response?.data?.message || e?.message || 'Nhập Excel thất bại'
        });
    } finally {
...
<template>
    <div class="px-4 md:px-6 lg:px-8 py-5 space-y-5">
        ...
        <div class="flex flex-wrap items-center gap-3 justify-between">
            <div class="flex flex-wrap items-center gap-2">
                ...
                <button
                    ...
                    :class="{ 'chip--active': statusFilter === 'neverLoggedIn' }"
                    @click="setStatusFilter('neverLoggedIn')"
                >
                    Chưa đăng nhập - {{ counterNever }}
                </button>
                <button
                ...
                >
                    Hoạt động - {{ counterActive }}
                </button>
                <button
                    class="chip chip--amber"
                    :class="{ 'chip--active': statusFilter === 'locked' }"
                    @click="setStatusFilter('locked')"
                >
                    Tạm khóa - {{ counterLocked }}
                </button>
                <button
                    class="chip chip--red"
                    :class="{ 'chip--active': statusFilter === 'deleted' }"
                    @click="setStatusFilter('deleted')"
                >
                    Đã bị xóa - {{ counterDeleted }}
                </button>
            </div>
        </div>
        ...
        >
            ...
            >
                <i class="fa-solid fa-spinner fa-spin mr-2"></i> Đang tải danh
                sách giáo viên...
            </div>
            ...
            <DataTable
                ...
                :rowHover="true"
                class="p-datatable-sm staff-table"
            >
                <Column
                ...
                <Column>
                    ...
                    <template #body="{ data }">
                        <div class="flex items-center gap-3">
                            ...
                            <Avatar
                                v-else
                                :label="(data.name?.[0] ?? 'N').toUpperCase()"
                                class="!bg-slate-100 !text-slate-700"
                                shape="circle"
                            />
                            <div class="min-w-0 leading-tight">
                                <div
                                    class="font-medium text-slate-900 break-any"
                                >
                                    {{ data.name || '-' }}
                                </div>
                                <div
                    ...
                    <template #header>
                        <div class="header-filter nowrap">
                            ...
                            <button
                                class="sort-btn"
                                @click="onSort('phone')"
                                :aria-label="'Sắp xếp số điện thoại'"
                            >
                                <i class="fa-solid fa-up-down"></i>
                            </button>
                        </div>
        ...
        >
            <div v-if="currentTeacher" class="space-y-2">
                <div class="flex items-center gap-3 mb-3">
                    <Avatar
                        v-if="currentTeacher.avatarUrl"
                        :image="currentTeacher.avatarUrl"
                        shape="circle"
                        size="large"
                    />
                    <Avatar
                        v-else
                        :label="(currentTeacher.name?.[0] ?? 'N').toUpperCase()"
                        shape="circle"
                        class="!bg-slate-100 !text-slate-700"
                        size="large"
                    />
        ...
        <Dialog
            ...
            modal
            header="Sửa thông tin giáo viên"
            :style="{ width: '500px' }"
        >
            <div class="space-y-3">
                <div>
        ...
        <!-- Dialog nhập giáo viên từ Excel -->
        <Dialog
            v-model:visible="showImportDialog"
            modal
            header="Nhập giáo viên từ Excel"
            :style="{ width: '500px' }"
        >
            <div class="space-y-3">
                <p class="text-sm text-slate-600">
                    Bước 1: Tải file Excel mẫu và điền thông tin giáo viên theo
                    cột.<br />
                    Bước 2: Chọn file đã điền và nhấn <b>Nhập dữ liệu</b>.
                </p>
                ...
                <div
                    v-if="importSummary"
                    class="mt-2 rounded-lg bg-slate-50 border border-slate-200 px-3 py-2 text-sm text-slate-700"
                >
                    <div>
                        Tổng bản ghi:
                        {{
                ...
                <div class="flex justify-end gap-2 mt-4">
                    ...
                    <Button
                        :disabled="importing"
                        :label="importing ? 'Đang nhập...' : 'Nhập dữ liệu'"
                        class="!bg-primary !border-0 !text-white"
