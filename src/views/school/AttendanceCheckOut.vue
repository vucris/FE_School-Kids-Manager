...
/* Chỉ được điểm danh về nếu học sinh có đi học (PRESENT, LATE, EARLY_LEAVE) */
function canCheckOut(row) {
    const st = row.status || 'UNDEFINED';
...
function toggleCheckOut(row) {
    // Không cho điểm danh về nếu học sinh nghỉ
    if (!canCheckOut(row)) {
        swalToast.fire({
            icon: 'info',
            title: 'Chỉ điểm danh về cho học sinh đi học'
        });
        return;
    }
...
/* Điểm danh tất cả học sinh đi học đang hiển thị là ĐÃ VỀ */
function markAllCheckedOut() {
    if (!filteredRows.value.length) {
        swalToast.fire({
            icon: 'info',
            title: 'Không có học sinh để điểm danh'
        });
        return;
    }

    const candidates = filteredRows.value.filter((r) => canCheckOut(r) && !isCheckedOut(r));

    if (!candidates.length) {
        swalToast.fire({
            icon: 'info',
            title: 'Tất cả học sinh phù hợp đã được điểm danh về'
        });
        return;
    }
    ...
    candidates.forEach((r) => {
        ...
        r.checkOutBy = checkedByUser.value;
        dirty.add(r.studentId);
    });

    swalToast.fire({
        icon: 'success',
        title: `Đã đánh dấu về cho ${candidates.length} học sinh`
    });
}
...
<template>
    <div class="px-4 md:px-6 lg:px-10 py-6 space-y-5 relative">
        ...
        <div class="flex flex-wrap items-center gap-3 text-sm text-slate-700">
            <span>
                <strong>{{ counts.total }}</strong> học sinh
            </span>
            <span>
                Đi học: <strong>{{ counts.presentCount }}</strong>
            </span>
            <span>
                Đã về: <strong>{{ counts.checkedOutCount }}</strong>
            </span>
        </div>
            ...
            <table class="min-w-full">
                    ...
                    <tr>
                        ...
                        <th class="th min-w-[120px] text-center">
                            <div class="flex items-center justify-center gap-2">
                                <span>Đã về ({{ counts.checkedOutCount }})</span>
                                <button type="button" class="btn-check-all" v-tooltip.top="'Đánh dấu đã về cho tất cả học sinh đi học đang hiển thị'" @click="markAllCheckedOut">
                                    <i class="fa-solid fa-user-check mr-1"></i>
                                    <span class="hidden md:inline">Tất cả</span>
                                </button>
                            </div>
                        </th>
                ...
                <tbody>
                    <tr v-for="(r, idx) in filteredRows" :key="r.studentId" class="hover:bg-slate-50 transition">
                        ...
                        <td class="td text-center">
                            <button
                                v-tooltip.top="canCheckOut(r) ? (isCheckedOut(r) ? 'Bỏ điểm danh về' : 'Đánh dấu đã về') : 'Học sinh nghỉ - không điểm danh về'"
                                class="btn-check"
                                :class="{
                                    'btn-check-on': isCheckedOut(r),
                                    'btn-check-disabled': !canCheckOut(r)
                                }"
                                :disabled="!canCheckOut(r)"
                                @click="toggleCheckOut(r)"
                            >
...
/* Nút điểm danh tất cả ở header ĐÃ VỀ */
.btn-check-all {
    display: inline-flex;
    align-items: center;
    justify-content: center;
...
/* Trạng thái disabled của nút ĐÃ VỀ */
.btn-check-disabled,
.btn-check:disabled {
    border-color: #e5e7eb;
    background: #f9fafb;
