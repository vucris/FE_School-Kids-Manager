...
const counts = computed(() => {
  ...
  const presentCount = rows.value.filter(
    (r) => (r.status || 'UNDEFINED') === 'PRESENT'
  ).length;
  ...
  <div class="px-4 md:px-6 lg:px-10 py-6 space-y-5 relative">
    ...
    <div class="flex border-b mb-4 gap-6 text-sm font-semibold">
      ...
      <RouterLink
        ...
        class="tab-link"
        :class="{ 'tab-link--active': route.path.includes('check-in') }"
      >
        Điểm danh đến
      </RouterLink>

      <RouterLink
        to="/school/attendance/check-out"
        class="tab-link"
        :class="{ 'tab-link--active': route.path.includes('check-out') }"
      >
        Điểm danh về
      </RouterLink>

      <RouterLink
        to="/school/attendance/leave-request"
        class="tab-link"
        :class="{ 'tab-link--active': route.path.includes('leave-request') }"
      >
        Đơn xin nghỉ
      </RouterLink>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-3 text-sm text-slate-700">
      <span><strong>{{ counts.total }}</strong> học sinh</span>
      <span>Đi học: <strong>{{ counts.presentCount }}</strong></span>
      <span>Đã về: <strong>{{ counts.checkedOutCount }}</strong></span>
    </div>
        ...
        <thead class="sticky top-0 z-10 bg-gradient-to-r from-slate-50 to-slate-100 border-b">
          <tr>
            ...
            <th class="th min-w-[120px] text-center">
              Đi học ({{ counts.presentCount }})
            </th>
            <th class="th min-w-[120px] text-center">
              Đã về ({{ counts.checkedOutCount }})
            </th>
        ...
        <tbody>
          ...
          >
            ...
            <td class="td text-center">
              ...
              <span
                v-else-if="r.status === 'LATE'"
                class="badge badge-late"
              >
                Đi muộn
              </span>
              <span
                v-else-if="r.status === 'EARLY_LEAVE'"
                class="badge badge-early"
              >
                Về sớm
              </span>
              <span v-else class="badge badge-undefined">
                Chưa xác định
              </span>
            </td>
            ...
            <td class="td text-center">
              <button
                v-tooltip.top="isCheckedOut(r) ? 'Bỏ điểm danh về' : 'Đánh dấu đã về'"
                class="btn-check"
                :class="{ 'btn-check-on': isCheckedOut(r) }"
                @click="toggleCheckOut(r)"
              >
                <i
                  class="fa-solid"
                  :class="isCheckedOut(r) ? 'fa-check' : 'fa-minus'"
                ></i>
              </button>
            </td>
            ...
            <td class="td">
              <div class="time-wrap" :class="{ 'opacity-40': !isCheckedOut(r) }">
                ...
                <input
                  class="time-display"
                  type="text"
                  :value="isCheckedOut(r) ? formatTime(r.checkOutTime) : ''"
                  disabled
                />
              </div>
            </td>
            ...
            <td class="td">
              <div v-if="isCheckedOut(r)" class="flex flex-col gap-1 text-slate-700">
                ...
                <div v-if="r.checkOutBy" class="text-sm">
                  Bởi {{ r.checkOutBy }}
                </div>
              </div>
            ...
            <td class="td">
              <InputText
                v-model="r.checkOutNote"
                class="w-full"
                placeholder="Ghi chú..."
                @input="onNoteChange(r)"
              />
            </td>
          </tr>
        </tbody>
